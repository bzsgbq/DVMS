...
'''
version 6 的motion map版本: 在version6的基础上, 不再采用saliency信息作为视点预测的辅助信息, 而是采用motion map;
...
class PerceiverMotPreprocessor(AbstractPreprocessor):
    ...
    def _build_network_inputs(self, inputs):  # inputs.shape = (B, T, 1, H, W); return.shape = (B, T, H*W+pos_dim)
        ...
        inputs = inputs.view(batch_size, T, -1)  # (B, T, H*W)
...
class PerceiverForViewpointPrediction6(PerceiverPreTrainedModel):
    def __init__(self, config, m_window, h_window, motmap_shape):
        ...
        self.m_window = m_window
        self.h_window = h_window
        self.motmap_shape = motmap_shape
            ...
            input_preprocessor=PerceiverMultimodalPreprocessor(
                ...
                modalities={
                    ...
                    "mot": PerceiverMotPreprocessor(
                        ...
                        m_window=self.m_window,
                        h_window=self.h_window,
                        feat_dim=motmap_shape[0]*motmap_shape[1]*3,
                        position_encoding_type="fourier",
                        concat_or_add_pos="concat",
                    ),
                },
...
class PerceiverVPMotmapModel6(nn.Module):
    def __init__(self, config, M_WINDOW, H_WINDOW, motmap_shape):
        super(PerceiverVPMotmapModel6, self).__init__()
        self.m_window = M_WINDOW
        self.h_window = H_WINDOW
        self.motmap_shape = motmap_shape
        ...
        self.perceiver = PerceiverForViewpointPrediction6(config, self.m_window, self.h_window, self.motmap_shape)
...
if __name__ == "__main__":
    ...
    model = PerceiverVPMotmapModel6(cfg, m_window, h_window, motmap_shape)
    pos = torch.ones(batch_size, m_window, 3)
    m_mot = 2 * torch.ones(batch_size, m_window, motmap_shape[0], motmap_shape[1], 3)
    h_mot = 2 * torch.ones(batch_size, h_window, motmap_shape[0], motmap_shape[1], 3)
