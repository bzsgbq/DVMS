...
'''
相比于perceiver6, error_perceiver6有以下区别: 
(1) decoder中的output_num_channels设为了1;
(2) (可选; 默认不选, 因为采用之后性能变差) 通过F.relu激活函数将logits限制为正数;
...
# 下面的ErrorPerceiverForViewpointPrediction6相比于perceiver6中的PerceiverForViewpointPrediction6, 唯一的区别是: 将decoder中的output_num_channels设为了1
class ErrorPerceiverForViewpointPrediction6(PerceiverPreTrainedModel):
...
class ErrorPerceiver6Model(nn.Module):
    def __init__(self, config, M_WINDOW, H_WINDOW, salmap_shape):
        super(ErrorPerceiver6Model, self).__init__()
        self.m_window = M_WINDOW
        self.h_window = H_WINDOW
        ...
        self.perceiver = ErrorPerceiverForViewpointPrediction6(config, self.m_window, self.h_window, num_data)
...
if __name__ == "__main__":
    ...
    model = ErrorPerceiver6Model(cfg, m_window, h_window, salmap_shape)
