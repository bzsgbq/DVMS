...
'''
version 3: 将pos和sal作为两种模态的输入, pos的shape为(batch_size, m_window, hidden_size), 
sal的shape为(batch_size, m_window+h_window, hidden_size); 也就是说, 相比于version 2, 我们
分别使用一个线性层, 将pos和sal的维度映射到hidden_size, 然后再输入到perceiver中.
...
class PerceiverForViewpointPrediction3(PerceiverPreTrainedModel):
    def __init__(self, config, m_window=5, h_window=25, hidden_size=512):
...
class PerceiverVPModel3(nn.Module):
    def __init__(self, config, M_WINDOW, H_WINDOW, salmap_shape, pos_dim=3, hidden_size=512):
        super(PerceiverVPModel3, self).__init__()
        self.m_window = M_WINDOW
        self.h_window = H_WINDOW
        self.pos_dim = pos_dim
        self.sal_dim = salmap_shape[0] * salmap_shape[1]
        self.hidden_size = hidden_size

        self.perceiver = PerceiverForViewpointPrediction3(config, m_window=self.m_window, h_window=self.h_window, hidden_size=self.hidden_size)
        self.fc_pos = nn.Linear(self.pos_dim, self.hidden_size)
        self.fc_sal = nn.Linear(self.sal_dim, self.hidden_size)
...
if __name__ == "__main__":
    ...
    model = PerceiverVPModel3(cfg, M_WINDOW=m_window, H_WINDOW=h_window, salmap_shape=salmap_shape)
    pos = torch.rand(batch_size, m_window, 3)
    m_sal = torch.rand(batch_size, m_window, 1, *salmap_shape)
    h_sal = torch.rand(batch_size, h_window, 1, *salmap_shape)
