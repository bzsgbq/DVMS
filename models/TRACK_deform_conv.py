...
try:
    ...
    from models.PAVER.model.vit import DeformPatchEmbed
except:
    from models_utils import LambdaLayer, toPosition, get_model_size
    from PAVER.model.vit import DeformPatchEmbed


class TRACKDeformConvModel(nn.Module):
    def __init__(self, M_WINDOW, H_WINDOW, salmap_shape, hidden_size=256, num_layers=1):
        super(TRACKDeformConvModel, self).__init__()
        ...
        self.sense_sal_enc = nn.LSTM(input_size=self.hidden_size, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True)#, dropout=0.5)
        ...
        self.sense_sal_dec = nn.LSTM(input_size=self.hidden_size, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True)#, dropout=0.5)
        ...
        self.deform_conv = DeformPatchEmbed(img_size=(salmap_shape[0], salmap_shape[1]), patch_size=(8, 8), in_chans=1, embed_dim=self.hidden_size//4, out_dim=self.hidden_size, norm_layer=nn.LayerNorm)
    ...
    def forward(self, x):
        ...
        # out_flat_enc = encoder_saliency_inputs.flatten(start_dim=0, end_dim=1)
        out_flat_enc = self.deform_conv(encoder_saliency_inputs)
        ...
        for curr_idx in range(self.h_window):
            ...
            flatten_timestep_saliency = self.deform_conv(selected_timestep_saliency)
...
if __name__ == "__main__":
    ...
    model = TRACKDeformConvModel(M_WINDOW, H_WINDOW, SALMAP_SHAPE)
