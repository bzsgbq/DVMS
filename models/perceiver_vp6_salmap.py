...
'''
version 6 的salmap版本: 在version6的基础上, 不再采用salxyz, 而是直接采用salmap;
            ...
            input_preprocessor=PerceiverMultimodalPreprocessor(
                ...
                modalities={
                    ...
                    "sal": PerceiverSalPreprocessor(
                        ...
                        m_window=self.m_window,
                        h_window=self.h_window,
                        feat_dim=salmap_shape[0]*salmap_shape[1],
                        position_encoding_type="fourier",
                        concat_or_add_pos="concat",
                    ),
                },
...
class PerceiverVPSalmapModel6(nn.Module):
    def __init__(self, config, M_WINDOW, H_WINDOW, salmap_shape):
        super(PerceiverVPSalmapModel6, self).__init__()
        self.m_window = M_WINDOW
        self.h_window = H_WINDOW
        self.salmap_shape = salmap_shape
        ...
        self.perceiver = PerceiverForViewpointPrediction6(config, self.m_window, self.h_window, self.salmap_shape)
...
if __name__ == "__main__":
    ...
    model = PerceiverVPSalmapModel6(cfg, m_window, h_window, salmap_shape)
    pos = torch.ones(batch_size, m_window, 3)
    m_sal = 2 * torch.ones(batch_size, m_window, 1, salmap_shape[0], salmap_shape[1])
    h_sal = 2 * torch.ones(batch_size, h_window, 1, salmap_shape[0], salmap_shape[1])
