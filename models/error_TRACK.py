...
'''
相比于TRACK，ErrorTRACKModel的区别在于：
(1) 将最后的fc_layer_out的输出维度改为1, 而不是3;
(2) decoding时, 将inputs改为零向量;
'''
...
class ErrorTRACKModel(nn.Module):
    def __init__(self, M_WINDOW, H_WINDOW, salmap_shape, hidden_size=256, num_layers=2):
        ...
        self.fc_layer_out = nn.Linear(in_features=self.hidden_size, out_features=1)
    ...
    def forward(self, x):
        ...
        # Decoding
        all_pos_outputs = []
        inputs = torch.zeros((decoder_position_inputs.shape[0], 1, 1), dtype=torch.float32, device=decoder_position_inputs.device)
...
if __name__ == "__main__":
    ...
    model = ErrorTRACKModel(M_WINDOW, H_WINDOW, SALMAP_SHAPE)
