...
class TRACKAblatAllModel(nn.Module):
    def __init__(self, M_WINDOW, H_WINDOW, salmap_shape, hidden_size=256, num_layers=1):
        ...
        self.fuse_2 = nn.Linear(in_features=2*self.hidden_size, out_features=self.hidden_size)
        ...
        if not USE_CNN:
            ...
            self.sal_convs = SalConvs(salmap_shape=salmap_shape, init_ch_size=8, output_size=self.hidden_size, num_conv_layer=3)
...
if __name__ == "__main__":
    ...
    model = TRACKAblatAllModel(M_WINDOW, H_WINDOW, SALMAP_SHAPE)
