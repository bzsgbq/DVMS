...
try:
    ...
    from models.perceiver_vp9 import CrossAttentionLayer, IdentityPreprocessor
except:
    ...
    from perceiver_vp9 import CrossAttentionLayer, IdentityPreprocessor


'''
version 10: 在version9的基础上, pos.shape=(B, T, 3), sal.shape=(B, T, N*4), 先用 pos 去 query sal, 然后得到的pos_sal的shape就是(B, T, N*4), 再将pos_sal和sal这两个模态送入perceiver;
...
if USE_FC:
    ...
    NUM_DATA = 2048  # saliency点的数量; (这里只是象征性地赋值, 实际上在在定义网络时会根据参数salmap_shape[0]来赋值)
...
class PerceiverPosPreprocessor(AbstractPreprocessor):
    ...
    def _build_network_inputs(self, inputs):
        ...
        pos_trainable = self.trainable_pos_encoder(batch_size)
...
class PerceiverForViewpointPrediction10(PerceiverPreTrainedModel):
    def __init__(self, config, m_window, h_window):
        ...
        self.perceiver = PerceiverModel(
            ...
            input_preprocessor=PerceiverMultimodalPreprocessor(
                ...
                mask_probs={"pos_sal": 0.0, "sal": 0.0},
            ),
...
class PerceiverVPModel10(nn.Module):
    def __init__(self, config, M_WINDOW, H_WINDOW, salmap_shape):
        super(PerceiverVPModel10, self).__init__()
        self.m_window = M_WINDOW
        self.h_window = H_WINDOW
        global NUM_DATA
        ...
        self.perceiver = PerceiverForViewpointPrediction10(config, self.m_window, self.h_window)
    ...
    def forward(self, x):
        ...
        # # 选择1:
        # enc_pos_sal_in = self.cross_attn_pos_sal(inputs=enc_pos_in, latents=enc_sal_in)
        # inputs = dict(pos_sal=enc_pos_sal_in, sal=enc_sal_in)
        # 选择2:
        enc_sal_pos_in = self.cross_attn_sal_pos(inputs=enc_sal_in, latents=enc_pos_in)
        inputs = dict(pos_sal=enc_pos_in, sal=enc_sal_pos_in)
...
if __name__ == "__main__":
    ...
    model = PerceiverVPModel10(cfg, m_window, h_window, salmap_shape)
