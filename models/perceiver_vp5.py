...
'''
version 5: 在version4的基础上, 采用processed salmap; 所谓processed salmap, 就是已经向原始的salmap中注入了pos信息, 且只保留saliency值最大的N个点 (比如取saliency值最大的20%的点, 且以5为步长去取)
...
class PerceiverForViewpointPrediction5(PerceiverPreTrainedModel):
...
class PerceiverVPModel5(nn.Module):
    def __init__(self, config, M_WINDOW, H_WINDOW):
        super(PerceiverVPModel5, self).__init__()
        self.m_window = M_WINDOW
        self.h_window = H_WINDOW

        self.perceiver = PerceiverForViewpointPrediction5(config, self.m_window, self.h_window)
...
if __name__ == "__main__":
    ...
    model = PerceiverVPModel5(cfg, m_window, h_window)
    pos = torch.ones(batch_size, m_window, 3)
    m_sal = 2 * torch.ones(batch_size, m_window, 2, 4)
    h_sal = 2 * torch.ones(batch_size, h_window, 2, 4)
