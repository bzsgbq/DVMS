...
'''
version 2: 将pos和sal作为两种模态的输入, pos的shape为(batch_size, m_window, 3), 
sal的shape为(batch_size, m_window+h_window, 64*128)
...
class PerceiverForViewpointPrediction2(PerceiverPreTrainedModel):
    def __init__(self, config, m_window=5, h_window=25, pos_dim=3, sal_dim=64*128):
        super().__init__(config)
        self.m_window = m_window
        self.h_window = h_window
        self.pos_dim = pos_dim
        self.sal_dim = sal_dim
...
class PerceiverVPModel2(nn.Module):
    def __init__(self, config, M_WINDOW, H_WINDOW, salmap_shape, pos_dim=3):
        super(PerceiverVPModel2, self).__init__()
        self.m_window = M_WINDOW
        self.h_window = H_WINDOW
        ...
        self.perceiver = PerceiverForViewpointPrediction2(config, m_window=self.m_window, h_window=self.h_window, pos_dim=self.pos_dim, sal_dim=self.sal_dim)
...
if __name__ == "__main__":
    ...
    model = PerceiverVPModel2(cfg, M_WINDOW=m_window, H_WINDOW=h_window, salmap_shape=salmap_shape)
