...
class CVPR18Model(nn.Module):
    def __init__(self, M_WINDOW, H_WINDOW, salmap_shape, hidden_size=256, num_layers=2):
        ...
        # self.fuse_1_dec = nn.LSTM(input_size=self.hidden_size+self.salmap_height*self.salmap_width, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True)
        self.fc_sal = nn.Linear(in_features=self.salmap_height*self.salmap_width, out_features=self.hidden_size)
        self.fuse_1 = nn.Linear(in_features=2*self.hidden_size, out_features=self.hidden_size)
...
if __name__ == "__main__":
    ...
    # encoder_saliency_inputs = torch.randn(B, M_WINDOW, *SALMAP_SHAPE)
    ...
    # decoder_saliency_inputs = torch.randn(B, H_WINDOW, *SALMAP_SHAPE)
    
    model = CVPR18Model(M_WINDOW, H_WINDOW, SALMAP_SHAPE)
