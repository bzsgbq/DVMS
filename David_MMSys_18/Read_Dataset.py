...
OUTPUT_FOLDER_SLOF_MOTION = './David_MMSys_18/motion_slof_big'
OUTPUT_FOLDER_PROCESSED_MOTION = './David_MMSys_18/motion_processed'
...
def get_traces_for_train_val_test():
    ...
    # # 2. 新划分方式: (测试是否是因为trainset和testset中视频的划分导致了过拟合)
    # videos_ids_train = [0, 3, 5, 7, 8, 4, 11, 14, 16, 15]
    # videos_ids_test = [1, 2, 9, 13, 18]

    # # region 方式1: 原方式: 训练集与测试集的视频和用户均不重叠; 验证集与测试集的视频一致, 与训练集的用户一致;
    # train_traces = []
    # for video_id in videos_ids_train:
    #     for user_id in users_train:
    #         train_traces.append({'video': videos[video_id], 'user': str(user_id)})
    ...
    # _videos, _users = videos_ids_test, users_test  # 方式1: 既划分用户, 又划分视频;
    # # _videos, _users = videos_ids_train, users_test  # 方式2: 只划分用户, 不划分视频;
    # # _videos, _users = videos_ids_test, users_train  # 方式3: 只划分视频, 不划分用户;
    # test_traces = []
    ...
    # # region 方式2: 不划分用户, 只划分视频;
    # video_ids = np.arange(len(videos))
    # np.random.shuffle(video_ids)
    # # 从19个视频中随机选择3个作为验证集, 4个作为测试集, 剩余12个作为训练集;
    # videos_ids_val = video_ids[:3]
    # videos_ids_test = video_ids[3:7]
    # videos_ids_train = video_ids[7:]
    ...
    #     videos_ids_val = videos_ids[:3]
    #     videos_ids_test = videos_ids[3:7]
    #     videos_ids_train = videos_ids[7:]
    ...
    # # region 方式5: 首先按照方式1, 使训练集与测试集的视频和用户均不重叠; 然后将 "训练集视频-测试集用户" 和 "训练集用户-测试集视频" 这两个被丢弃掉的数据部分也放入训练接. 验证集与测试集一致;
    # train_traces = []
    # for video_id in videos_ids_train:
    #     for user_id in users_train:
    #         train_traces.append({'video': videos[video_id], 'user': str(user_id)})
...
def split_traces_and_store():
    ...
    store_dict_as_csv('David_MMSys_18/train_set', ['user', 'video'], train_traces)
    store_dict_as_csv('David_MMSys_18/val_set', ['user', 'video'], val_traces)
    store_dict_as_csv('David_MMSys_18/test_set', ['user', 'video'], test_traces)
