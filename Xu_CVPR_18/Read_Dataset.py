...
import numpy as np
import pandas as pd
from utils.utils import eulerian_to_cartesian, cartesian_to_eulerian, get_xyz_grid, salmap2posalfeat, rotationBetweenVectors, interpolate_quaternions, degrees_to_radian, radian_to_degrees, store_dict_as_csv
from pyquaternion import Quaternion
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D
from utils.sampled_dataset import get_video_ids, get_user_ids, get_users_per_video
import cv2
import argparse
...
OUTPUT_FOLDER_TRUE_SALIENCY = './Xu_CVPR_18/saliency_true'
NUM_TILES_WIDTH_TRUE_SAL = 256
NUM_TILES_HEIGHT_TRUE_SAL = 256

OUTPUT_FOLDER_PAVER_SALIENCY = './Xu_CVPR_18/saliency_paver_big'
OUTPUT_FOLDER_PROCESSED_SALIENCY = './Xu_CVPR_18/saliency_processed'
...
def create_and_store_processed_saliency():
    os.makedirs(OUTPUT_FOLDER_PROCESSED_SALIENCY, exist_ok=True)
    videos = get_video_ids(OUTPUT_FOLDER)
    for enum_video, video in enumerate(videos):
        print('creating processed saliency for video', video, '-', enum_video, '/', len(videos))
        paver_sal_path = os.path.join(OUTPUT_FOLDER_PAVER_SALIENCY, video+'.npy')
        paver_sal = np.load(paver_sal_path)  # (NUM_FRAMES, 224, 448)

        # # plot:
        # raw_salmap = paver_sal[444]
        ...
        # 需要将paver_saliency设置为paver_big; 其每张salmap的形状为(224, 448)
        H, W = paver_sal.shape[-2:]
        num_data = 20480  # 20480 / (224*448) = 0.20408...; 即取每张salmap中saliency值最大的的前20%个数据
        stride = 40  # 以40的步长取数据
        xyz_grid = get_xyz_grid(H, W).numpy()  # (H, W, 3)
        processed_sal = salmap2posalfeat(xyz_grid, paver_sal)  # (NUM_FRAMES, H*W, 4)
...
def get_traces_for_train_val_test():
    ...
    videos = get_video_ids(OUTPUT_FOLDER)
    users = get_user_ids(OUTPUT_FOLDER)
    ...
    for video_id in videos:
        for user_id in users_train:
            if user_id in users_per_video[video_id]:
...
def split_traces_and_store():
    ...
    store_dict_as_csv('Xu_CVPR_18/train_set', ['user', 'video'], train_traces)
    store_dict_as_csv('Xu_CVPR_18/val_set', ['user', 'video'], val_traces)
    store_dict_as_csv('Xu_CVPR_18/test_set', ['user', 'video'], test_traces)
